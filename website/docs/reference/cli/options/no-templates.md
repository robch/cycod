...
The `--no-templates` option disables template processing in all text inputs to CycoD.
...
By default, CycoD processes templates in inputs, allowing for variable substitution, conditionals, and other template features. The `--no-templates` option turns this feature off completely, causing all curly braces and template expressions to be treated as literal text.
...
```bash
cycod --no-templates --input "function example() { return {id: 1, name: 'Test'}; }"
```
...
```bash
cycod --no-templates --question "Explain the following CSS: .container { display: flex; justify-content: center; }"
```
...
```bash
cycod --no-templates --input @config_template.txt --question "Help me understand this configuration template"
```
...
```bash
cycod --var name=Alice --no-templates --input "How to use {name} as a variable in templates?"
```
...
- [Templates and Variables](../../../usage/templates-and-variables.md) - Detailed guide on using templates in CycoD