...
# Using Azure OpenAI with CycoD

This tutorial will guide you through setting up and using Azure OpenAI with CycoD, including how to configure and work with different model deployments.
...
- The CycoD CLI tool installed on your machine

## Setting Up Azure OpenAI with CycoD
...
### Step 2: Configure CycoD with Azure OpenAI Settings
...
```bash
cycod --question "What's your name and which model are you using?"
```
...
In Azure OpenAI, a "deployment" is an instance of a specific model that you've deployed to your Azure OpenAI resource. The `--azure-openai-chat-deployment` parameter is used to specify which deployment CycoD should use.
...
### Specifying Deployments with CycoD

The `--azure-openai-chat-deployment` option lets you select which deployment to use:

```bash
cycod --use-azure-openai --azure-openai-chat-deployment your-deployment-name --question "Hello!"
```
...
cycod --profile azure-gpt4 --question "Explain quantum computing to me"

# Use the GPT-3.5 profile for simpler tasks
cycod --profile azure-gpt35 --question "What's the capital of France?"
```
...
1. **"Deployment not found" error**
   ...
   cycod config get azure.openai.chatDeployment --any
   ```
...
- Set up CycoD to use Azure OpenAI
...
- Explore [CycoD profiles](../advanced/profiles.md) for more advanced configuration
- Check out how to use [CycoD with GitHub Copilot](github-copilot-auth.md) as an alternative provider