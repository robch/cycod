...
Aliases are named shortcuts for ChatX command options. Instead of typing a long command with multiple options, you can save those options as an alias and use the alias name prefixed with `--`.

For example, instead of typing:

```bash
chatx --use-openai --openai-chat-model-name gpt-4o --add-system-prompt "You are an expert Python programmer."
```

You can create an alias:

```bash
chatx --use-openai --openai-chat-model-name gpt-4o --add-system-prompt "You are an expert Python programmer." --save-alias python-expert
```

And then use it simply as:

```bash
chatx --python-expert --question "How do I implement a decorator in Python?"
```

## Creating Aliases

ChatX provides several options for creating aliases at different scopes:
...
chatx [options] --save-alias my-alias

# Explicitly create a local alias
chatx [options] --save-local-alias my-alias
```
...
```bash
chatx [options] --save-user-alias my-alias
```
...
chatx --add-system-prompt "You are a writing assistant who helps improve clarity, flow, and grammar." --save-user-alias writer

# Create a personal data analysis assistant
chatx --add-system-prompt "You are a data analysis expert who helps interpret data, suggest visualization approaches, and identify meaningful patterns." --save-user-alias data-analyst
```
...
chatx [options] --save-global-alias my-alias
```
...
```bash
chatx --my-alias --question "What is the capital of France?"
```

## Managing Aliases

ChatX provides commands to list, view, and delete aliases:
...
```bash
chatx alias get my-alias --local
chatx alias get my-alias --user
chatx alias get my-alias --global
```
...
```bash
chatx alias delete my-alias --local   # Delete from local scope only
chatx alias delete my-alias --user    # Delete from user scope only
chatx alias delete my-alias --global  # Delete from global scope only
chatx alias delete my-alias --any     # Delete from any scope (default behavior)
```

When an alias is successfully deleted, the command will display the path of the deleted file:

```
Deleted: C:\Users\username\.chatx\aliases\my-alias.alias
```
...
Located in the `.chatx/aliases` directory of your current working directory:
...
chatx --add-system-prompt "You're familiar with our project structure and coding standards." --save-local-alias project-assistant
```
...
chatx --add-system-prompt "You are a translation expert. Translate between languages accurately and naturally." --save-user-alias translator
```
...
chatx --use-openai --openai-chat-model-name gpt-4o --add-system-prompt "You are a security expert. Analyze code for security vulnerabilities following our organization's standards." --save-global-alias security-review
```
...
- [Understanding Scopes in ChatX](scopes.md)
...
- [chatx alias delete Command](/reference/cli/alias/delete.md)
- [chatx alias get Command](/reference/cli/alias/get.md)
- [chatx alias list Command](/reference/cli/alias/list.md)
