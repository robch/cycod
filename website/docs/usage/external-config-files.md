...
```bash
chatx --config project-config.yaml --question "Explain Docker containers"
```
...
```bash
chatx --config base.yaml project.yaml --question "What's new in Python 3.11?"
```
...
chatx --config base-config.yaml secrets.yaml --question "Generate a project plan"
```
...
chatx --config base.yaml dev.yaml --question "Debug this code"

# Production environment
chatx --config base.yaml prod.yaml --question "Analyze production logs"
```
...
```bash
chatx --config team-settings.yaml personal-settings.yaml --question "Review this PR"
```
...
chatx --config base.yaml code-gen.yaml --question "Write a Python class for user authentication"

# For content creation tasks
chatx --config base.yaml content.yaml --question "Write a blog post about AI"
```
...
Usage:
```bash
chatx --config openai.yaml --question "Explain quantum computing"
```
...
Usage:
```bash
chatx --config azure.yaml --question "Explain quantum computing"
```
...
Usage:
```bash
chatx --config project.yaml --question "Explain the project architecture"
```
...
Usage:
```bash
chatx --config normal.yaml debug.yaml --question "Why is this failing?"
```
...
app:
  ...
  historyDirectory: "//server/chatx-logs"
```
...
4. Local scope configuration (`.chatx/config.json` in current directory)
5. User scope configuration (`.chatx/config.json` in user's home directory)
6. Global scope configuration (`.chatx/config.json` in system-wide location)
...
CHATX supports both external configuration files (`--config`) and profiles (`--profile`). Here's when to use each:
...
chatx --profile work --config project-specific.yaml --question "Explain microservices"
```
...
3. Verify the configuration hierarchy matches CHATX's expected structure
