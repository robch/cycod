...
Custom prompts are reusable templates that help streamline common interactions with CycoD.
...
When you use a slash command for a prompt in interactive mode, CycoD now immediately replaces the command with the prompt content and sends it to the assistant. This provides a smoother experience when working with custom prompts.
...
cycod prompt create hello "Hello! How can I help you today?"
```

``` { .plaintext .cli-output }
Prompt 'hello' saved to: C:\Users\username\.cycod\prompts\hello.prompt
```
...
=== "PowerShell"
    ...
    cycod prompt create code-review "Please review this code and suggest improvements:`n1. Identify any bugs or edge cases`n2. Suggest performance optimizations`n3. Comment on style and readability"
    ```

``` { .plaintext .cli-output }
Prompt 'code-review' saved to: C:\Users\username\.cycod\prompts\code-review.prompt
```
...
=== "Bash"
    ...
    cycod prompt create commit-msg @commit-msg-prompt.txt
    ```
...
``` { .plaintext .cli-output }
Prompt 'commit-msg' saved to: C:\Users\username\.cycod\prompts\commit-msg.prompt
```
...
Prompts are stored as text files in your CycoD user directory:
...
??? tip "Sharing Prompts with Your Team"
    ...
    CycoD to look for prompts in that location using the `--config-dir` option or by setting up
...
cycod --inputs "/code-review" "function getUser(id) { return users.find(u => u.id === id); }" "/refactor"
```
...
cycod prompt create translate "Translate the following text from {source_lang} to {target_lang}:

{text}"
```

``` { .plaintext .cli-output }
Prompt 'translate' saved to: C:\Users\username\.cycod\prompts\translate.prompt
```
...
CycoD provides a convenient `--prompt` alias for working with custom prompts. This alias works similarly to `--add-user-prompt` but is specifically designed for seamless integration with the custom prompt system.
...
cycod --add-user-prompt "/code-review" --input "function sum(a, b) { return a + b; }"
```
