...
CycoD provides a flexible configuration system that helps you customize its behavior at different scopes.
...
LOCATION: C:\ProgramData\.cycod\config.yaml (global)

  No configuration settings found.

LOCATION: C:\Users\username\.cycod\config.yaml (user)

  No configuration settings found.

LOCATION: C:\project\.cycod\config (local)
...
??? tip "Understanding Configuration Scopes"
    ...
    - **Local** - Settings for the current directory (`.cycod/config.yaml`)
    - **User** - Settings for the current user (`~/.cycod/config.yaml`)
    - **Global** - Settings for all users on the system (`/etc/cycod/config.yaml` or `C:\ProgramData\.cycod\config.yaml`)
...
cycod config get github.token --user
```
...
cycod config set app.preferredProvider copilot
```
...
cycod config set app.maxTokens 4000 --user
```
...
cycod config set app.trimTokenTarget 32000 --global
```
...
cycod config set app.allowedDomains example.com --user
```
...
cycod config add app.allowedDomains trusted-site.org --user
```
...
cycod config set openai.apiKey sk-yourapikeyhere --user
...
cycod config set azure.openai.apiKey yourazureapikey --user
...
Create a YAML profile file in `.cycod/profiles/`:
...
CycoD uses YAML configuration files. Here's an example of what a typical configuration file might look like:

```yaml title="Example configuration file (.cycod/config.yaml)"
app:
...
??? tip "Configuration Precedence Example"
    ...
    ```bash
    # In your user config file (~/.cycod/config.yaml)
    copilot:
      modelName: claude-3.5-sonnet
    
    # In a profile file (.cycod/profiles/work.yaml)
    copilot:
      modelName: claude-3.7-sonnet
    ...
    # On the command line
    cycod --copilot-model-name claude-3.7-opus --profile work
    ```
