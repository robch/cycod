...
/// </summary>
public class UnifiedShellAndProcessHelperFunctions
{
        ...
        [Description("Maximum total number of characters to display.")] int maxTotalChars = 100000)
    {
        ...
        try
        {
            ...
            var shellResult = NamedShellManager.Instance.CreateShell(parsedShellType, tempShellName, workingDir, envVars);
            ...
            var result = await NamedShellManager.Instance.ExecuteInShellAsync(tempShellName, command, expectedTimeout);

            // If command timed out, promote the shell to a named shell
            if (result.TimedOut)
            {
                // Auto-promote to a named shell
                string promotedShellName = NamedShellManager.Instance.CreateAutoPromotedShell(parsedShellType, command, workingDir);
                
                // Terminate the temporary shell
                NamedShellManager.Instance.TerminateShell(tempShellName);
        ...
        [Description("Environment variables as JSON object")] string environmentVariables = "")
    {
        ...
        try
        {
            var result = NamedShellManager.Instance.CreateShell(parsedShellType, shellName, workingDirectory, envVars);
        ...
        [Description("Environment variables as JSON object")] string environmentVariables = "")
    {
        ...
        try
        {
            var result = EnhancedBackgroundProcessManager.StartNamedProcess(
                executablePath,
                processName,
                processArguments,
                workingDirectory,
        ...
        [Description("Maximum total number of characters to display.")] int maxTotalChars = 100000)
    {
        ...
        try
        {
            var result = await NamedShellManager.Instance.ExecuteInShellAsync(shellName, command, timeoutMs);
            return TextTruncationHelper.TruncateOutput(result.ToAiString(), maxCharsPerLine, maxTotalChars);
        }
        catch (Exception ex)
        {
        ...
        [Description("Maximum total number of characters to display.")] int maxTotalChars = 100000)
    {
        ...
        try
        {
            ...
            var shell = NamedShellManager.Instance.GetShell(name);
            ...
            if (shell != null)
            {
                ...
                {
                    // Wait for pattern in shell output
                    string patternOutput = await NamedShellManager.Instance.WaitForShellOutputAsync(
                        name, waitPattern, waitTimeMs > 0 ? waitTimeMs : -1);
                    ...
                    // Get the full output after pattern match
                    var cmdResult = await NamedShellManager.Instance.ExecuteInShellAsync(
                        name, "echo \"\"", 5000); // Just to get current output
                    ...
                    return TextTruncationHelper.TruncateOutput(outputResult.ToAiString(), maxCharsPerLine, maxTotalChars);
                }
                else if (waitTimeMs > 0)
                {
                    // Just wait for some time to get more output
                    await Task.Delay(waitTimeMs);
                }
                
                // Execute empty command to get current output state
                var result = await NamedShellManager.Instance.ExecuteInShellAsync(
                    name, "echo \"\"", 5000);
            ...
            else
            {
                ...
                if (EnhancedBackgroundProcessManager.IsProcessRunning(name))
                {
                    if (!string.IsNullOrEmpty(waitPattern))
                    {
                        // Wait for pattern in process output
                        bool matched = EnhancedBackgroundProcessManager.WaitForProcessOutput(
                            name, waitPattern, waitTimeMs > 0 ? waitTimeMs : -1, out string patternOutput);
                    ...
                    // Get process output
                    var outputResult = EnhancedBackgroundProcessManager.GetProcessOutput(name, outputType, clearBuffer);
                    return TextTruncationHelper.TruncateOutput(outputResult.ToAiString(), maxCharsPerLine, maxTotalChars);
                }
                else
                {
        ...
        [Description("Text to send as input")] string inputText)
    {
        ...
        try
        {
            ...
            {
                // This is a shell
                bool success = NamedShellManager.Instance.SendInputToShellAsync(name, inputText).Result;
                return success 
            ...
            else
            {
                ...
                if (EnhancedBackgroundProcessManager.IsProcessRunning(name))
                {
                    bool success = EnhancedBackgroundProcessManager.SendInputToProcess(name, inputText);
                    return success 
        ...
        [Description("Maximum total number of characters to display.")] int maxTotalChars = 100000)
    {
        ...
        try
        {
            ...
            if (shell != null)
            {
                ...
                string result = await NamedShellManager.Instance.WaitForShellOutputAsync(
        ...
        [Description("Maximum total number of characters to display.")] int maxTotalChars = 100000)
    {
        ...
        try
        {
            ...
            if (shell != null)
            {
                ...
                var result = NamedShellManager.Instance.TerminateShell(name, force);
                return TextTruncationHelper.TruncateOutput(result.ToAiString(), maxCharsPerLine, maxTotalChars);
            }
            else
            {
                ...
                if (EnhancedBackgroundProcessManager.IsProcessRunning(name))
                {
                    var result = EnhancedBackgroundProcessManager.TerminateProcess(name, force);
                    return TextTruncationHelper.TruncateOutput(result.ToAiString(), maxCharsPerLine, maxTotalChars);
                }
                else
                {
        ...
        [Description("Maximum total number of characters to display.")] int maxTotalChars = 100000)
    {
        try
        {
            var shellNames = NamedShellManager.Instance.GetAllShellNames();
            var processNames = EnhancedBackgroundProcessManager.GetAllProcessNames();
            ...
            {
                var shellInfo = NamedShellManager.Instance.GetShellInfo(shellName);
                result += $"ID: {shellName}\n";
            ...
            // Add processes
            foreach (var processInfo in EnhancedBackgroundProcessManager.GetAllProcessInfo())
            {
