...
public class ChatCommand : CommandWithVariables
{
    ...
    private void HandleUpdateMessages(IList<ChatMessage> messages)
    {
        ...
        // Generate title after first meaningful exchange (unless disabled)
        var autoGeneratedTitlesDisabled = ConfigStore.Instance.GetFromAnyScope("App.AutoGeneratedTitlesDisabled").AsBool(defaultValue: false);
        var shouldGenerate = _currentChat?.Conversation.NeedsTitleGeneration() == true;
        
        ConsoleHelpers.WriteDebugLine($"Title generation check: attempted={_titleGenerationAttempted}, shouldGenerate={shouldGenerate}, autoGeneratedTitlesDisabled={autoGeneratedTitlesDisabled}, messageCount={messages.Count}");
        
        if (!_titleGenerationAttempted && shouldGenerate && !autoGeneratedTitlesDisabled)
        {
